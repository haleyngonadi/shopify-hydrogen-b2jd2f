  ...
  const filterMarkup = (filter, option) => {
    switch (filter.type) {
      ...
      default:
        ...
        return (
          <Link
            className={`pr-6 py-3 flex grow relative text-xs break-words focus:underline hover:underline ${
              isActive ? 'font-bold' : ''
            }`}
            prefetch="intent"
            to={to}
          ...
          {filters.map(
            (filter) =>
              filter.values.length > 0 && (
                ...
                >
                  <Menu.Button className="inline-flex w-full justify-between lg:justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-100">
                    {filter.label}
                    <svg
                  ...
                  >
                    <Menu.Items className="lg:absolute relative left-0 mt-2 w-full lg:w-72 origin-top-left divide-y divide-gray-100 border border-slate-100 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                      <div className="hidden lg:flex items-center p-4 border-b border-b-slate-100">
                        <SelectedFilters
...
function SelectedFilters({filters = [], filter}) {
  const location = useUrl();
  const params = new URLSearchParams(location.search);
  let maxPrice = 0;

  if (filter.label == 'Price') {
    const {price} = JSON.parse(filter.values[0].input);
    maxPrice = price.max;
  }
      ...
      <div className="flex flex-wrap gap-2">
        {filters.map((filter) => {
          return (
            ...
            >
              <span className="flex-grow mr-1">{filter.label}</span>
              <span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
...
function PriceRangeFilter({max, min}) {
  ...
  return (
    <div className="flex items-center lg:w-auto w-full justify-center lg:py-0 py-3">
      <label className="inline-flex items-center">
        <span className="mr-2">$</span>
        <input
          name="maxPrice"
          className="text-black border-slate-300 border w-24 py-2 px-2"
          type="text"
          defaultValue={min}
      ...
      <label className="inline-flex items-center">
        ...
        <input
          name="minPrice"
          className="text-black border-slate-300 border py-2 w-24 px-2"
          type="number"
          defaultValue={max}
